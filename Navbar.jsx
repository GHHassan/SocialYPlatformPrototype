...
import { useMainAuth } from "../mainAuth";
import toast from "react-hot-toast";

const Navbar = () => {
  const { authState, setSignedIn } = useMainAuth();
  ...
  return (
    <header className="flex flex-wrap sm:justify-start sm:flex-nowrap z-50 w-full bg-accent text-sm py-4">
      <nav
        ...
        >
          <div className="flex flex-col gap-y-4 gap-x-0 mt-5 sm:flex-row sm:items-center sm:justify-end sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:ps-7">
            ...
            {authState &&
              ...
              authState.user.role === "Researcher" && (
                <Link
                  to="/create-listing"
                  className="font-medium hover:text-gray-300 text-white"
                >
                  <p className="text-nowrap">Create Listing</p>
                </Link>
              )}
